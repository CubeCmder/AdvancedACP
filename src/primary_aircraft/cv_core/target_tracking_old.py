...
def target_tracking(frame, color, nb_detect, lat_PA, lon_PA, compass_theta, K_it, vect_GPS_target, prev, frame_rate):
    ...
    if time_elapsed > 1. / frame_rate:
        ...
        # perimter of the target knowing its real dimension
        r = 1.3  # cm
        ...
        if not contours:
            ...
            cX, cY = np.nan, np.nan
            GPS_lat_target = np.nan
            GPS_lon_target = np.nan
            north_x_target = np.nan
            north_y_target = np.nan
            vect_GPS_target[K_it] = np.array([np.nan, np.nan])
            # print("frame", K_it)
            # print("no contour")
            pass
    ...
    return (GPS_lat_target, GPS_lon_target, vect_GPS_target, K_it, north_x_target, north_y_target)
