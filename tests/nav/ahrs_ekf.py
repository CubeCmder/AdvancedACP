...
if __name__ == '__main__':
    ...
    wmm.I  # Magnetic declination [degrees]
    ...
    acc_data = [0]*num_samples
    gyr_data = [0]*num_samples
    mag_data = [0]*num_samples
    a0 = [imu.readACCx()*g, imu.readACCy()*g, imu.readACCz()*g]
    Q[0] = acc2q(a0)  # First sample of tri-axial accelerometer
    ekf = EKF(frequency=frequency, magnetic_ref=wmm.I ,q0=[0.7071, 0.0, -0.7071, 0.0])  # Assuming sensors have 1000 samples each
    print(a0)
    ...
    try:
        for t in range(1, num_samples):
            ...
            print(f'ACC: {acc_data[t]}')
            print(f'GYR: {gyr_data[t]}')
            print(f'MAG: {mag_data[t]}\n')

            Q[t] = Quaternion(ekf.update(Q[t - 1], gyr=gyr_data[t], acc=acc_data[t]), mag=mag_data, dt=1/frequency)
            print(Q[t])
